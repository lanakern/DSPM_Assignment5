...
# if your structure of your api file is different, please store it here under my
# name "api_key". It is often used in the code. 
...
full_pages <- as.numeric(floor(n/size_200))  # 24
full_pages

# how many venues are on last page (page 25)?
results_last_page <- n - (full_pages * size_200)  # 238
results_last_page
...
resp_venues_ger_lastpage <-
  GET(url = "https://app.ticketmaster.com/discovery/v2/venues.json?",
      query = list(
        ...
        locale = "*" ,                # use * to match all locales
        size = size_200               # adjust number of entries per page
      )
  )
...
```{r ex6_austria}
# extract data
data_netherlands <- func_venues("NL", 500, api_key)

# show structure of data
str(data_netherlands)

# create map (extreme points are extracted from Wikipedia)
func_map(data_netherlands, "Netherlands", 50.750417, 53.465556, 3.358333, 7.227778)
```
